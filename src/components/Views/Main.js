...
const Main=()=>{
    ...
    const {user,isInitialize} = useSelector(state=>state); 
    ...
    useEffect(()=>{
        ...
        // const getIP = async () => {
        //     await userService.getIPAddress(); 
        // }
        Promise.all([getCategories(),getUser(),getAllProduct(),getCart()]).then((values)=>dispatch({type:'initialize'}));
        // getCategories();
        // getUser();
        // getAllProduct();
        // getCart();
